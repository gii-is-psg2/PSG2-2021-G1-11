...
@WebMvcTest(value = OwnerController.class, includeFilters = @ComponentScan.Filter(value = PetTypeFormatter.class, type = FilterType.ASSIGNABLE_TYPE),
...
class OwnerControllerTests {
	...
	void testProcessFindFormSuccess() throws Exception {
		BDDMockito.given(this.ownerService.findOwnerByLastName("")).willReturn(Lists.newArrayList(this.george, new Owner()));
	...
	void testProcessFindFormByLastName() throws Exception {
		BDDMockito.given(this.ownerService.findOwnerByLastName(this.george.getLastName())).willReturn(Lists.newArrayList(this.george));
	...
	void testRemoveOwner() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.post("/owners/{ownerId}/remove", OwnerControllerTests.TEST_OWNER_ID).with(SecurityMockMvcRequestPostProcessors.csrf())).andExpect(MockMvcResultMatchers.status().is3xxRedirection())
	...
	void testObtainMyProfile() throws Exception {
		BDDMockito.given(this.ownerService.getOwnerByUserName(ArgumentMatchers.any(String.class))).willReturn(new Owner());
		BDDMockito.given(this.adoptionApplicationService.getPendingAdoptionApplication(ArgumentMatchers.any(Owner.class))).willReturn(new ArrayList<AdoptionApplication>());
		this.mockMvc.perform(MockMvcRequestBuilders.get("/owners/myProfile")).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.view().name("owners/ownerDetails"));
	}
