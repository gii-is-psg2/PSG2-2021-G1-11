...
class OwnerControllerTests {
	...
	void testObtainMyProfile() throws Exception {
		given(ownerService.getOwnerByUserName(any(String.class))).willReturn(new Owner());
		given(adoptionApplicationService.getPendingAdoptionApplication(any(Owner.class)))
				.willReturn(new ArrayList<AdoptionApplication>());
		mockMvc.perform(get("/owners/myProfile")).andExpect(status().isOk())
				.andExpect(view().name("owners/ownerDetails"));
	}
